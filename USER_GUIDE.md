...
- `Native` means the required Alert Call prompts exist and the locale resolves to its own playback language, including complete generic or regional English and French profiles. `Fallback` means an unsupported locale resolves to a validated English Alert Call language. `Unavailable` means a supported locale candidate does not have its complete native prompt set. Active-row highlighting and resolver candidate ordering do not determine the displayed status.
