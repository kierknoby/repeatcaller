...
- The Alert Call Language Status table in Global Settings shows each relevant PBX language, whether its exact locale is installed, its supported profile status, and the native or English fallback behavior Alert Call will use. The active language row is highlighted, and installed files do not promote a language into support.
