...
- The Alert Call Language Status table in Global Settings shows each relevant PBX language, whether its exact locale is installed, the language Alert Call will actually use, and its status. `Active / Preferred` means the active FreePBX language uses its preferred Alert Call language; `Active / Fallback` means it uses English fallback; `Fallback` identifies the configured English fallback; `Native` identifies an installed supported language; and `Fallback only` identifies an installed language that will use the configured English fallback. The active row is highlighted. Each row's Sample play button rotates through Repeat, Invert, and acceptance audio in the displayed Alert Call language.
