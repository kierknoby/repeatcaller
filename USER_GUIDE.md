...
- **Status:** Whether the language is native, active fallback, fallback, or fallback only.
...
- `Native` means the required Alert Call prompts exist and will be played using that language. `Active / Preferred` means the active FreePBX language uses its preferred Alert Call language; `Active / Fallback` means it uses English fallback; `Fallback` identifies the configured English fallback; and `Fallback only` identifies an installed language that will use the configured English fallback.
