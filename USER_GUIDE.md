...
- **Status:** Whether the locale resolves natively, the active locale uses fallback, a non-active locale is fallback only, or playback is unavailable.
...
- A locale can exist and contain audio files while its required Alert Call prompts remain incomplete, so it may still use fallback behaviour.
...
- `Native` means the required Alert Call prompts exist and the locale resolves to its own playback language, including complete generic or regional English and French profiles. `Active / Fallback` means the active FreePBX locale resolves to another language, while `Fallback only` means a non-active locale resolves to another language. `Active/unavailable` and `Unavailable` mean no usable Alert Call language is available. Resolver candidate ordering does not determine the displayed status.
