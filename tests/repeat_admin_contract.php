...
assert_true(strpos($jsSource, "currentBulkEngineAction = 'resume';") !== false && strpos($jsSource, "buttonText = 'Resume';") !== false, 'snoozed monitoring state should expose Resume as the single bulk action');
assert_true(strpos($jsSource, "currentBulkEngineAction = 'disable';") !== false && strpos($jsSource, "buttonText = 'Disable Rules';") !== false, 'enabled unsnoozed monitoring state should expose Disable Rules as the single bulk action');
assert_true(strpos($jsSource, "currentBulkEngineAction = 'enable';") !== false && strpos($jsSource, "buttonText = 'Enable Rules';") !== false, 'disabled monitoring state should expose Enable Rules as the single bulk action');
...
assert_true(strpos($jsSource, "callerIdHelpText = 'Enter Caller ID in E.164 format, e.g. ' + e164Example + '.';") !== false, 'Alert Call Caller ID helper should request E.164 input using the dynamic default-country example');
...
assert_true(strpos($jsSource, 'var alertCallSelfTriggerWarning = ') !== false && strpos($jsSource, 'Alert Call destinations are automatically added to Ignore these callers') !== false, 'rule editor should define the one-time Alert Call self-trigger warning text');
