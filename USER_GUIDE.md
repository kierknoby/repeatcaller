...
- `Native` means the locale has a complete required Alert Call prompt set and Alert Call Language matches the locale, including complete generic or regional English and French profiles. `Unavailable` means the locale cannot provide native Alert Call playback, including incomplete and unsupported locales. Alert Call Language still shows the language that will actually play. Active-row highlighting and resolver candidate ordering do not determine the displayed status.
