...
63. Open the rule editor and confirm DID Scope offers `Include All DIDs` (`all`) and `Select DIDs` (`selected`). Include All DIDs must grey out the full route editor without clearing either list and must match every route. Select DIDs must re-enable both visible lists and independent Include Route and Exclude Route actions. Empty lists must display the visual defaults `All DIDs` and `No DIDs`; an empty include list means all routes before exclusions, and exclusion wins if a route is in both lists. Confirm `Select an inbound route…` is initially selected, the first real route works immediately, successful actions reset the selector, duplicates remain blocked, and both lists survive scope changes and save/reload. On upgrade, confirm legacy All-DIDs rules with exclusions become Select DIDs while preserving those exclusions and effective matching.
