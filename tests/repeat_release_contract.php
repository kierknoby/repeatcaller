...
assert_true(strpos($readmeSource, '**Release date:** 23 August 2026') !== false, 'README should declare the 1.0.2 release date');
...
assert_true(strpos($readmeSource, '## Alert Call Language Detection') !== false, 'README should include dedicated Alert Call language detection guidance');
assert_true(strpos($readmeSource, '/var/lib/asterisk/sounds/<locale>') !== false, 'README should explain how to remove a leftover locale directory');
assert_true(strpos($readmeSource, 'Fallback selection behaviour is unchanged.') !== false, 'README should clarify fallback status does not change selection behavior');
