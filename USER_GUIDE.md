...
- The standalone Alert Call language capability section separately shows maintainer-supported profiles, exact installed PBX language directories, missing required prompts, and the active native/fallback selection. Maintainers decide which profiles are officially supported; installed files do not promote a language into support.
