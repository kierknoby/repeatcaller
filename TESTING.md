...
63. Open the rule editor and confirm both Included Routes and Excluded Routes remain visible. Confirm Inbound Routes initially shows `Select an inbound route…` with Include Route and Exclude Route disabled. Select the first real route and verify either action immediately adds it to the corresponding list, resets the selector, blocks duplicates, and survives save/reload.
