...
class Repeatcaller implements \BMO {
	...
	private function alertCallLanguageStatusRows(\FreePBX\modules\Repeatcaller\AlertCallLanguageSupport $support, array $status, string $activeLanguage): array {
		...
		foreach (array_values($locales) as $locale) {
			...
			foreach ((array)($status['supported_profiles'] ?? []) as $profile) {
				if ($this->containsAlertCallLocale((array)($profile['locale_candidates'] ?? []), $locale)) {
			...
			if ($resolvesToOwnLocale) {
				$statusLabel = _('Native');
			} elseif ($usesFallback && !$supportedLocale) {
